...
std::string Bitcoin::pubkeyToAddr(int type, Point& pubKey, bool isCompressed)
{
	...
	switch (type) { 
		...
		case BECH32:
			...
			char addr_bech32[128];
			// segwit_addr_encode(char *output, const char *hrp, int witver, const uint8_t *witprog, size_t witprog_len)
			segwit_addr_encode(addr_bech32, "bc", 0, address + 1, 20);
			return std::string(addr_bech32);
	}
...
void hiiu_decodeBase58(std::string address, uint32_t* _hash160){
	...
	// memcpy(_hash160, hash160Keccak + 1, 4);
	// memcpy(_hash160 + 1, hash160Keccak + 5, 4);
	// memcpy(_hash160 + 2, hash160Keccak + 9, 4);
	// memcpy(_hash160 + 3, hash160Keccak + 13, 4);
	// memcpy(_hash160 + 4, hash160Keccak + 17, 4);
	
	//print
	// printf("\n");
	// for (int i = 0; i < 5; i++){	printf("\n ==> test_hash160[%d] : %d ",i , hash160Keccak[i]); };
}
...
void hiiu_decodeBech32(const char* addr,  uint32_t* _hash160) {
	printf("\n\n ---- hiiu_decodeBech32(_h[5]) : "); std::cout << addr;
	...
	// memcpy(_hash160, hash, 4);
	// memcpy(_hash160 + 1, hash + 4, 4);
	// memcpy(_hash160 + 2, hash + 8, 4);
	// memcpy(_hash160 + 3, hash + 12, 4);
	// memcpy(_hash160 + 4, hash + 16, 4);
}
...
// // 	/////////////////////////////
	hiiu_decodeBase58("13zb1hQbWVsc2S7ZTZnP2G4undNNpdh5so", h);
...
// // 	/////////////////////////////
	hiiu_decodeBase58("138XxHZGcKM6WyWuYCijLsoCd8K3x4WYjs", h);
...
// // 	/////////////////////////////
	hiiu_decodeBase58("3PdQoXyQwWmerpt3SbF7Hbh3aukC5w28GP", h);
...
// // 	/////////////////////////////
	hiiu_decodeBech32("bc1qyr2956nky56hqr8fuzepdccejse4mw994lyftn", h);
